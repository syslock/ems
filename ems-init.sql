...
CREATE TABLE objects (id INTEGER PRIMARY KEY, type TEXT, read NUMERIC, write NUMERIC, sequence NUMERIC NOT NULL DEFAULT 0, mtime NUMERIC);
CREATE TABLE titles (object_id NUMBER, data TEXT);
CREATE TABLE applications (object_id NUMERIC, user_id NUMERIC, statement_id NUMERIC, motivation_id NUMERIC, motto_id NUMERIC, university_name TEXT, study_field TEXT, study_years NUMERIC, study_finish NUMERIC, topic_id1 NUMERIC, topic_id2 NUMERIC, topc_id3 NUMERIC, accommodation TEXT, food TEXT, comment TEXT, abstract_id NUMERIC, cultural_id NUMERIC, conferences_id NUMERIC);
